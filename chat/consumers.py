...
class ContractConsumer(AsyncWebsocketConsumer):
    ...
    def create_chat_message(self, room_id, sender_id, contract_type):
        ...
        ChatMessage.objects.create(
            room=room_obj, user=user_obj, contract_type=contract_type, application=True)
        if contract_type == "수락" or contract_type == "거절":
            contract_message = ChatMessage.objects.get(
                Q(room=room_obj) & ~Q(user=user_obj) & Q(contract_type="신청"))
